...
```
Again, one line! Another advantage about using vectorization in `numpy` is multiplying matrices. Let's take the previus list of lists `a`, and multiply it by itself, or compute $\text{a}^2$. 
